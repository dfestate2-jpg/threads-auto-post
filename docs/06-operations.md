...
> ⚠️ **プルリクエストのプレビューではマイグレーションを実行しない。**
> Netlify のデプロイプレビューは本番と同じ環境変数（＝本番DB）を見るため、
> ここで `prisma migrate deploy` を走らせると
> **マージされていないPRのマイグレーションが本番DBに適用されてしまう。**
> `netlify.toml` の `[context.deploy-preview]` でビルドコマンドを `npm run build` に
> 切り替えてあり、プレビューはDBに接続しない（`next build` はDB接続を行わない）。
