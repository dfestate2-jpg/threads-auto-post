...
> このダッシュボードは追客管理システムと同じアプリに `/sentiment` 配下として同居している。
> テーブルもジョブも共有しておらず、追客側の画面には影響しない。
> 画面はアプリ共通のログイン（`src/middleware.ts`）の内側にある。ログイン無しで見せたい場合は
> middleware の `matcher` から `sentiment` を除外する（`/api/markets` は元から認証の対象外）。
...
スケジュールは `vercel.json` に `*/20 * * * *` で入れてある。
ただし **このリポジトリの実際のデプロイ先は Netlify**（`netlify.toml` + `@netlify/plugin-nextjs`）で、
Netlify では `vercel.json` の `crons` は読まれない。Netlify で自動実行したい場合は、
追客側と同じく `netlify/functions/` に Scheduled Function を 1 つ足して
`/api/cron/collect` を叩く必要がある（この PR では追加していない）。
...
src/
  app/
    sentiment/            TOP / 詳細（この配下だけがダッシュボードの画面）
    api/markets/          銘柄 API
    api/cron/collect/     定期取得
  components/
    sentiment/            カード・バッジ・チャートなどの表示部品
  lib/
    markets.ts            対象銘柄の定義
