...
| [**14. Company OS**](docs/14-company-os.md) | **会社の立ち上げから経営までを一元管理する画面群（設計・優先順位の決め方・テンプレート）** |
...
### Company OS（会社経営の一元管理）
- `/company-os` … 会社の立ち上げから経営までを 1 か所で管理する
- **経営ダッシュボード**：今日の最重要 TOP5／期限超過・今日・今週／会社設立の進捗／経営課題・未決事項／経営数値／AIによる経営診断
- **タスク**：担当・期限・依存関係・チェックリスト・コメント。優先順位は期限順ではなく、
  重要度・他タスクへの影響・売上インパクト・リスクから算出し、**その理由も表示する**
- **プロジェクト**：進捗率と遅延をタスクから自動算出。マイルストーンのタイムライン
- **経営課題 / 意思決定 / 未決事項 / 会議ログ**：起きた問題 → 決めたこと → まだ決まっていないこと、を
  相互にリンクして残す。意思決定は変更履歴つき
- **会社立ち上げテンプレート**：ボタン1つで 45 タスクを順番と期限つきで作成（押し直しても増えない）
- **議事録の自動仕分け**：議事録から決定事項・タスク・課題・未決事項を抽出し、**確認してから**登録
- **AI経営アシスタント**：会社のデータを読んで「今日やるべきこと」「いま一番まずいこと」を答える
  （Phase1 はルールベース。外部への送信なし）
- 実装範囲・データ構造・優先順位の計算式は [docs/14-company-os.md](docs/14-company-os.md)
...
│   ├── company-os/           ★ Company OS（ダッシュボード・タスク・プロジェクト・
│   │                           経営課題・意思決定・未決事項・会議・会社情報・KPI・検索・AI）
│   └── api/company-os/       Company OS の API（汎用リソース＋個別の操作）
├── lib/
...
│   ├── company-os/           ★ Company OS の副作用なしロジック（テスト対象）
│   │   ├── priority.ts         優先順位エンジン（期限・重要度・依存・売上・リスク）
│   │   ├── advisor.ts          経営診断とAIアシスタントの応答
│   │   ├── minutes.ts          議事録から決定・タスク・課題・未決事項を仕分ける
│   │   ├── templates.ts        会社立ち上げ／会社買収テンプレート
│   │   ├── progress.ts         進捗率と遅延判定
│   │   ├── access.ts           会社内の権限・情報の公開範囲
│   │   └── date.ts             期限の扱い（タイムゾーン込み）
│   ├── services/companyOs/   Company OS のDB操作
│   ├── line/                 Messaging API クライアント・署名検証
│   ├── notify/               通知チャネルのディスパッチ（Slack / Discord / LINE WORKS / LINE）
│   └── auth/                 セッション・パスワード
├── components/               画面コンポーネント
│   ├── followup/               今日やること・アクションボタン・顧客登録・設定
│   └── company-os/             サイドバー・共通フォーム・タスク一覧・議事録の仕分け
...
├── company-os-demo.ts        Company OS の動作確認用データ
