...
const 設定 = {
  ...
  // 摘要の「どこかに含まれていたら」除外
  除外_含む: ['利息', 'リソク', 'ポイント', 'POINT', '取消', 'デビット'],
...
/**
 * シート名を「その月を指しているか」で比べられる形にする。
 *
 *   2026/09 → 2026-09     2026/9  → 2026-09
 *   202609  → 2026-09     2026-09 → 2026-09
 *   2026年9月 → 2026-09   "2026/09 " → 2026-09（前後の空白は無視）
 *
 * 月次シートに見えないもの（コピー / 大和リビング専用 など）は '' を返す。
 *
 * 完全一致だけで探すと、ゼロ埋めの有無や余分な空白だけで
 * **同じ月のシートを二重に作ってしまう**。それを防ぐために使う。
 */
function 月キー_(シート名) {
  const s = String(シート名 || '').normalize('NFKC').replace(/[　\s]+/g, '');
...
function 月シート取得_(d) {
  ...
  // ② 表記ゆれを許して探す（2026/9 や 202609 も同じ月とみなす）
  const 目標 = 月キー_(名前);
  if (目標) {
    const 候補 = ss.getSheets().filter(function (s) { return 月キー_(s.getName()) === 目標; });
    if (候補.length === 1) return 候補[0];
    if (候補.length > 1) {
      // どちらに書くべきか決められない。勝手に選ばず止める
      throw new Error(
        '同じ月（' + 目標 + '）のシートが複数あります: '
        + 候補.map(function (s) { return s.getName(); }).join(' / ')
        + '。1つに整理してから再実行してください。');
    }
  }
