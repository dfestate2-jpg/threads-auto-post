...
export function ChannelTable({
  ...
  /**
   * グループ人数の保存。
   * LINEの通数は人数分カウントされるので、ここが空だとダッシュボードの
   * 残量が実際より多く見え、気づいたときには上限、ということが起きる。
   */
  async function saveMemberCount(id: string, raw: string) {
    const value = raw.trim() === "" ? null : Number(raw);
    if (value !== null && (!Number.isInteger(value) || value < 0)) return;
    setRows((prev) =>
      prev.map((r) => (r.id === id ? { ...r, memberCount: value } : r)),
    );
    const res = await fetch(`/api/channels/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ memberCount: value }),
    });
    setMsg(res.ok ? "人数を保存しました" : "人数を保存できませんでした");
    router.refresh();
  }
  ...
  async function test(id: string) {
    ...
    setMsg(
      data.ok
        ? "✅ テスト送信に成功しました"
        : `❌ 送信失敗: ${data.results?.[0]?.error ?? "不明なエラー"}`,
    );
  }
    ...
    <section className="card p-4">
      ...
      {rows.length === 0 && !envGroupConfigured ? (
        <p className="mb-3 rounded bg-red-50 px-3 py-2 text-sm text-red-800">
          ⚠️
          通知先が1件も登録されていません。担当者未設定の顧客のリマインドが届きません。
        </p>
      ) : null}
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-b border-slate-100 last:border-0">
              ...
              <td
                className="max-w-[240px] truncate font-mono text-xs text-slate-500"
                title={r.target}
              >
                {r.target}
              </td>
              <td className="text-xs">
                {PURPOSES.find((p) => p.value === r.purpose)?.label}
              </td>
              <td className="text-xs">
                {r.type === "LINE_GROUP" ? (
                  <input
                    className={`input w-20 ${r.memberCount ? "" : "border-orange-300 bg-orange-50"}`}
                    type="number"
                    min={0}
                    placeholder="未登録"
                    defaultValue={r.memberCount ?? ""}
                    onBlur={(e) => void saveMemberCount(r.id, e.target.value)}
                    title="このグループの人数。LINEの通数は人数分カウントされます"
                  />
                ) : (
      ...
      <div className="mt-4 flex flex-wrap items-end gap-2 border-t border-slate-100 pt-4">
        ...
        <div>
          ...
          <select
            ...
            value={draft.type}
            onChange={(e) =>
              setDraft({ ...draft, type: e.target.value as ChannelRow["type"] })
            }
          >
            {TYPES.map((t) => (
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setDraft({
                ...draft,
                purpose: e.target.value as ChannelRow["purpose"],
              })
            }
          >
            {PURPOSES.map((p) => (
