...
import { getMonthlyUsage } from "@/lib/services/messageUsage";
...
export default async function ReminderDashboardPage() {
  ...
  const now = new Date();
  const [stats, worst, staff, usage] = await withReadRetry(() =>
    Promise.all([
      ...
      getMonthlyUsage(settings.timezone, settings.lineMonthlyFreeQuota, now),
    ]),
  );
    ...
    <AppShell alert={quotaAlert ?? cronAlert}>
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard
          ...
          href={stats.awaitingTotal > 0 ? "/reminders/unreplied" : undefined}
        />
        <StatCard
          label="🟠 1時間以上未返信"
          value={stats.over1h}
          tone={stats.over1h > 0 ? "warn" : "neutral"}
          href={stats.over1h > 0 ? "/reminders/unreplied?over=60" : undefined}
        />
        <StatCard
          label="🔴 3時間以上未返信"
          value={stats.over3h}
          tone={stats.over3h > 0 ? "danger" : "neutral"}
          href={stats.over3h > 0 ? "/reminders/unreplied?over=180" : undefined}
        />
        <StatCard
          label="🔴 24時間以上未返信"
          value={stats.over24h}
          ...
          href={
            stats.over24h > 0 ? "/reminders/unreplied?over=1440" : undefined
          }
        />
      </section>
      ...
      <section className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <StatCard
          ...
          href={
            stats.needsCheck > 0
              ? "/reminders/unreplied?status=NEEDS_CHECK&awaiting=0"
              : undefined
          }
        />
        <StatCard
          ...
          {worst.length === 0 ? (
            ...
            <CustomerRows
              rows={worst.map((c) => ({
                ...
                elapsedMinutes: c.firstUnrepliedAt
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <section className="card p-4">
              ...
              <ul className="space-y-2">
                {stats.byAssignee.map((a) => (
                  ...
                  >
                    ...
                    <span className="rounded bg-slate-100 px-2 py-0.5 font-semibold tabular-nums">
                      {a.count}
                    </span>
                  </li>
                ))}
            ...
            <p className="mt-4 border-t border-slate-100 pt-3 text-xs text-slate-500">
              最終Cron実行：
              {formatDateTimeJa(stats.cron.lastRunAt, settings.timezone)}
              {stats.cron.lastError ? (
                <span className="text-red-600">
                  {" "}
                  / エラー: {stats.cron.lastError}
                </span>
              ) : null}
            </p>
          </section>

          <MessageUsageCard usage={usage} />
        </div>
      </div>
    </AppShell>
  );
